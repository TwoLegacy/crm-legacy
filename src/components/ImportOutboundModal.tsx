...
import { FonteOutbound, importOutboundLeads } from '@/lib/outbound'

interface ImportOutboundModalProps {
  isOpen: boolean
  onClose: () => void
...
const FONTES_OUTBOUND: { value: FonteOutbound; label: string }[] = [
...
export default function ImportOutboundModal({ isOpen, onClose, onSuccess, userId }: ImportOutboundModalProps) {
  const [step, setStep] = useState<ImportStep>('upload')
  const [fonteOutbound, setFonteOutbound] = useState<FonteOutbound>('google_maps')
  ...
  const handleImport = async () => {
    ...
    try {
      setProgress(30)
      const importResult = await importOutboundLeads(parsedRows, fonteOutbound, userId)
      setProgress(100)
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto animate-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900">Importar Leads Outbound</h2>
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {FONTES_OUTBOUND.map(f => (
                    <button
                      key={f.value}
                      onClick={() => setFonteOutbound(f.value)}
                      className={`px-4 py-2.5 rounded-xl text-sm font-medium transition-all border ${
                        fonteOutbound === f.value
          ...
          {step === 'preview' && (
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-2 text-sm text-gray-500">
                ...
                {fileName} · Fonte: <span className="font-medium text-gray-700">{FONTES_OUTBOUND.find(f => f.value === fonteOutbound)?.label}</span>
              </div>
