...
import { FonteOutbound, createOutboundLead, OutboundLead } from '@/lib/outbound'

interface AddOutboundLeadModalProps {
  isOpen: boolean
  onClose: () => void
  onSuccess: (lead: OutboundLead) => void
  userId: string
}

const FONTES: { value: FonteOutbound; label: string }[] = [
...
export default function AddOutboundLeadModal({ isOpen, onClose, onSuccess, userId }: AddOutboundLeadModalProps) {
  ...
  const [form, setForm] = useState({
    ...
    link_perfil: '',
    fonte_outbound: 'google_maps' as FonteOutbound,
  })
  ...
  const handleSubmit = async () => {
    ...
    try {
      const lead = await createOutboundLead({
        ...
        fonte_outbound: form.fonte_outbound,
      }, userId)
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold text-gray-900">Novo Lead Outbound</h2>
          </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {FONTES.map(f => (
                <button
                  key={f.value}
                  type="button"
                  onClick={() => handleChange('fonte_outbound', f.value)}
                  className={`px-4 py-2.5 rounded-xl text-sm font-medium transition-all border ${
                    form.fonte_outbound === f.value
