...
import { OutboundLead, FonteOutbound, updateOutboundLead } from '@/lib/outbound'

interface LeadOutboundDetailModalProps {
  lead: OutboundLead | null
  onClose: () => void
  onUpdate: (updated: OutboundLead) => void
}
...
export default function LeadOutboundDetailModal({ lead, onClose, onUpdate }: LeadOutboundDetailModalProps) {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <InfoItem label="Fonte">{FONTE_LABELS[lead.fonte_outbound]}</InfoItem>
            <InfoItem label="Status">
              <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${
                lead.status_outbound === 'PROSPECTADO' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'
              }`}>
                {lead.status_outbound === 'PROSPECTADO' ? 'Prospectado' : 'Para Prospectar'}
              </span>
            </InfoItem>
          </div>
